export function myAvro(element, ...args) {
    if ("bangla" in element) return;
    element.bangla = true;
...
function mykeydown(e) {
    ...
    if (keycode === 77 && e.ctrlKey && !e.altKey && !e.shiftKey) {
        e.preventDefault();
        e.target.bangla = !e.target.bangla;
        return false;
    }
    if (!this.bangla) return;
    if ([32, 13, 9].includes(keycode)) myreplace(e.target);
}

function myreplace(el) {
    const cur = mygetCaret(el);
    const last = myfindLast(el, cur);
    const bangla = OmicronLab.Avro.Phonetic.parse(el.value.substring(last, cur));
    ...
    if (document.selection) {
        ...
        el.value =
        el.value.substring(0, last) +
        bangla +
        el.value.substring(cur);
        el.selectionStart = el.selectionEnd = cur - (Math.abs(cur - last) - bangla.length);
    }
}

function myfindLast(el, cur) {
    let last = cur - 1;
    while (last > 0) {
